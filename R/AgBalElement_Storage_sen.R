...
"AgBal" %>% PluckBind_sen() %>% 
  ...
  ## Bind storage ----
# bind_rows(
# "AgStorageClose" %>% PluckBind_sen() %>%
#   transmute(branch, scenario, year,region, sector, value, element = "ClosingStockBeforeLoss") %>%
#   bind_rows("AgStorageOpen" %>% PluckBind_sen()  %>% transmute(branch, scenario, year, region, sector, value, element = "Opening stocks")) %>%
#   bind_rows("AgStorageCloseLoss" %>% PluckBind_sen()  %>% transmute(branch, scenario, year,region, sector, value, element = "Closing stocks")) %>%
...
AgElement_SUA %>% 
  ...
  left_join_error_no_match(
    "Agprices" %>% PluckBind_sen() %>% #  1975$/kg
